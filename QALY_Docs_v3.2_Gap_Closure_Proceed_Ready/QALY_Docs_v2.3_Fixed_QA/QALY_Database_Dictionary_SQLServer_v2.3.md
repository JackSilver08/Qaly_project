# QALY Workspace — Database Dictionary & Data Model v2.3

**Phiên bản:** v2.3 — SQL Server aligned production specification  
**Ngày:** 16/05/2026  
...
- **SQL Server 2022 là database implementation target chính** cho đồ án QALY, khớp ASP.NET Core + EF Core + Docker dev environment.
- Các ghi chú PostgreSQL cũ chỉ còn giá trị tham chiếu lịch sử; khi code/demo dùng file `QALY_v2_1_SQLServer_Schema_Skeleton.sql` và mapping SQL Server dưới đây.
- Mapping chính: `UUID` → `UNIQUEIDENTIFIER`, `TIMESTAMPTZ` → `DATETIME2(7)` hoặc `DATETIMEOFFSET(7)` khi cần timezone tuyệt đối, `JSONB` → `NVARCHAR(MAX)` kèm `ISJSON`, `INET` → `VARCHAR(45)`, partial index → filtered index.
...
|---|---|---|---|---|
| `id` | UNIQUEIDENTIFIER | NOT NULL | NEWSEQUENTIALID() | Primary key |
...
| `metadata_json` | NVARCHAR(MAX) | NULL |  | Thông tin mở rộng có kiểm soát, dùng khi schema flexible |
| `created_by` | UNIQUEIDENTIFIER | NULL |  | User tạo bản ghi |
| `updated_by` | UNIQUEIDENTIFIER | NULL |  | User cập nhật gần nhất |
| `created_at` | DATETIME2(7) | NOT NULL | SYSUTCDATETIME() | Thời điểm tạo |
| `updated_at` | DATETIME2(7) | NOT NULL | SYSUTCDATETIME() | Thời điểm cập nhật |
| `deleted_at` | DATETIME2(7) | NULL | NULL | Soft delete |
...
| `status` | VARCHAR(40) | NULL |  | Trạng thái nghiệp vụ |
| `metadata_json` | NVARCHAR(MAX) | NULL |  | Thông tin mở rộng có kiểm soát, dùng khi schema flexible |
| `created_at` | DATETIME2(7) | NOT NULL | SYSUTCDATETIME() | Thời điểm tạo log |
| `archived_at` | DATETIME2(7) | NULL | NULL | Thời điểm archive theo retention nếu có |
...
|---|---|---|---|---|
| `id` | UNIQUEIDENTIFIER | NOT NULL | NEWSEQUENTIALID() | Primary key |
| `tenant_id` | UNIQUEIDENTIFIER | NOT NULL |  | FK tenants.id, phục vụ multi-tenant isolation |
| `name` | VARCHAR(200) | NULL |  | Tên hiển thị hoặc nhãn nghiệp vụ |
| `code` | VARCHAR(100) | NULL |  | Mã định danh tùy chọn |
| `status` | VARCHAR(40) | NULL |  | Trạng thái nghiệp vụ |
| `metadata_json` | NVARCHAR(MAX) | NULL |  | Thông tin mở rộng có kiểm soát, dùng khi schema flexible |
...
| `id` | UNIQUEIDENTIFIER | NOT NULL | NEWSEQUENTIALID() | Primary key |
| `tenant_id` | UNIQUEIDENTIFIER | NOT NULL |  | FK tenants.id, phục vụ multi-tenant isolation |
| `organization_id` | UNIQUEIDENTIFIER | NULL |  | FK organizations.id nếu bản ghi thuộc organization cụ thể |
| `user_id` | UNIQUEIDENTIFIER | NULL |  | FK users.id nếu bản ghi gắn với một user |
...
| `owner_id` | UNIQUEIDENTIFIER | NOT NULL |  | FK users.id của org_owner |
| `metadata_json` | NVARCHAR(MAX) | NULL |  | Thông tin mở rộng có kiểm soát, dùng khi schema flexible |
| `created_by` | UNIQUEIDENTIFIER | NULL |  | User tạo bản ghi |
| `updated_by` | UNIQUEIDENTIFIER | NULL |  | User cập nhật gần nhất |
| `created_at` | DATETIME2(7) | NOT NULL | SYSUTCDATETIME() | Thời điểm tạo |
...
| `owner_id` | UNIQUEIDENTIFIER | NOT NULL |  | PM/owner chính |
...
| `id` | UNIQUEIDENTIFIER | NOT NULL | NEWSEQUENTIALID() | Primary key |
| `tenant_id` | UNIQUEIDENTIFIER | NOT NULL |  | FK tenants.id, phục vụ multi-tenant isolation |
| `project_id` | UNIQUEIDENTIFIER | NULL |  | FK projects.id nếu bản ghi thuộc project |
| `name` | VARCHAR(200) | NULL |  | Tên hiển thị hoặc nhãn nghiệp vụ |
| `code` | VARCHAR(100) | NULL |  | Mã định danh tùy chọn |
| `status` | VARCHAR(40) | NULL |  | Trạng thái nghiệp vụ |
| `metadata_json` | NVARCHAR(MAX) | NULL |  | Thông tin mở rộng có kiểm soát, dùng khi schema flexible |
...
| `organization_id` | UNIQUEIDENTIFIER | NULL |  | FK organizations.id nếu bản ghi thuộc organization cụ thể |
| `project_id` | UNIQUEIDENTIFIER | NULL |  | FK projects.id nếu bản ghi thuộc project |
| `task_id` | UNIQUEIDENTIFIER | NULL |  | FK tasks.id nếu bản ghi gắn với task |
| `name` | VARCHAR(200) | NULL |  | Tên hiển thị hoặc nhãn nghiệp vụ |
| `code` | VARCHAR(100) | NULL |  | Mã định danh tùy chọn |
| `status` | VARCHAR(40) | NULL |  | Trạng thái nghiệp vụ |
| `metadata_json` | NVARCHAR(MAX) | NULL |  | Thông tin mở rộng có kiểm soát, dùng khi schema flexible |
...
| `due_at` | DATETIME2(7) | NULL |  | Deadline |
| `start_at` | DATETIME2(7) | NULL |  | Ngày bắt đầu |
...
| `parent_task_id` | UNIQUEIDENTIFIER | NULL |  | FK tasks.id cho subtask/epic |
| `created_by` | UNIQUEIDENTIFIER | NULL |  | User tạo bản ghi |
| `updated_by` | UNIQUEIDENTIFIER | NULL |  | User cập nhật gần nhất |
| `created_at` | DATETIME2(7) | NOT NULL | SYSUTCDATETIME() | Thời điểm tạo |
| `updated_at` | DATETIME2(7) | NOT NULL | SYSUTCDATETIME() | Thời điểm cập nhật |
...
| `tenant_id` | UNIQUEIDENTIFIER | NOT NULL |  | FK tenants.id, phục vụ multi-tenant isolation |
| `project_id` | UNIQUEIDENTIFIER | NULL |  | FK projects.id nếu bản ghi thuộc project |
| `room_id` | UNIQUEIDENTIFIER | NOT NULL |  | FK chat_rooms.id |
| `sender_id` | UNIQUEIDENTIFIER | NOT NULL |  | FK users.id |
...
| `pinned_at` | DATETIME2(7) | NULL |  | Thời điểm pin |
| `created_by` | UNIQUEIDENTIFIER | NULL |  | User tạo bản ghi |
| `updated_by` | UNIQUEIDENTIFIER | NULL |  | User cập nhật gần nhất |
| `created_at` | DATETIME2(7) | NOT NULL | SYSUTCDATETIME() | Thời điểm tạo |
| `updated_at` | DATETIME2(7) | NOT NULL | SYSUTCDATETIME() | Thời điểm cập nhật |
...
| `tenant_id` | UNIQUEIDENTIFIER | NOT NULL |  | FK tenants.id, phục vụ multi-tenant isolation |
| `project_id` | UNIQUEIDENTIFIER | NULL |  | FK projects.id nếu bản ghi thuộc project |
| `space_id` | UNIQUEIDENTIFIER | NULL |  | FK wiki_spaces.id |
| `parent_page_id` | UNIQUEIDENTIFIER | NULL |  | Trang cha |
...
| `id` | UNIQUEIDENTIFIER | NOT NULL | NEWSEQUENTIALID() | Primary key |
| `tenant_id` | UNIQUEIDENTIFIER | NOT NULL |  | FK tenants.id, phục vụ multi-tenant isolation |
| `source_file_id` | UNIQUEIDENTIFIER | NULL |  | FK files.id |
...
- Chat/presence realtime không lưu typing vào DB lâu dài; `realtime_typing_indicators` chỉ là mô hình Redis key, không phải bảng SQL Server bắt buộc.


---

## Phụ lục v2.3 — SQL Server alignment note

Tài liệu này đã được rà soát lại ở v2.3 để tránh hiểu nhầm rằng PostgreSQL là database triển khai chính. SQL Server 2022 là target triển khai; Qdrant vẫn là vector store cho AI/RAG. Những tên kiểu dữ liệu còn mang tính business/semantic trong ví dụ JSON như `uuid` chỉ biểu diễn identifier, không phải chỉ thị dùng PostgreSQL.
