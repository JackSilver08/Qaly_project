...
import type { ChatGroupModel, TeamChatAttachment, TeamChatMessage, TeamChatPoll } from './chat-types'
...
const emit = defineEmits<{
  send: [payload: { text: string; attachments: TeamChatAttachment[]; poll?: TeamChatPoll }]
...
<template>
  <section class="team-chat-window glass-card">
    ...
    <div v-if="pinnedMessages.length" class="team-pinned-list">
      <span v-for="message in pinnedMessages" :key="message.id">{{ message.text || message.poll?.question }}</span>
    </div>
    ...
    <div v-if="showPoll" class="team-poll-composer">
      <input v-model="pollQuestion" type="text" placeholder="Câu hỏi bình chọn" />
      <input v-for="(_, index) in pollOptions" :key="index" v-model="pollOptions[index]" type="text" :placeholder="`Lựa chọn ${index + 1}`" />
      <button class="text-button" type="button" @click="pollOptions.push('')">Thêm lựa chọn</button>
    </div>

    <div v-if="pendingAttachments.length" class="team-attachment-preview">
      <span v-for="file in pendingAttachments" :key="file.name">{{ file.name }}</span>
    </div>
    ...
    <form class="team-chat-composer" @submit.prevent="sendMessage">
      ...
      <button class="icon-button icon-button--small" type="button" aria-label="Emoji" @click="showEmoji = !showEmoji">
        <SmilePlus :size="16" />
      </button>
      <button class="icon-button icon-button--small" type="button" aria-label="Tạo poll" @click="showPoll = !showPoll">
        <Vote :size="16" />
      </button>
    ...
    <div v-if="showEmoji" class="team-emoji-picker glass-card">
      <button v-for="emoji in emojiOptions" :key="emoji" type="button" @click="addEmoji(emoji)">{{ emoji }}</button>
    </div>
  </section>
</template>
