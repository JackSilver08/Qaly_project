...
test('TEST-AI-NATIVE-TASK-HUB-E2E manager launcher navigates to canonical task context with a prepared prompt', async ({ page }) => {
  ...
  await expect(page.locator('.task-detail__header')).toContainText(managedTask.title)
  await expect(page.locator('.task-detail__project')).toHaveText(managedProject.name)
