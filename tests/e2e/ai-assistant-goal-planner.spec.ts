...
test('TEST-GS-E2E reload renders goal, selected skill, work plan and safe activity', async ({ page }) => {
  ...
  const firstPlanDisclosure = dialog.locator('details.assistant-work-plan-details').first()
  await expect(firstPlanDisclosure).not.toHaveAttribute('open', '')
  await firstPlanDisclosure.locator(':scope > summary').click()
  ...
  const firstProcessDisclosure = dialog.locator('details.assistant-process-disclosure').first()
  await expect(firstProcessDisclosure).not.toHaveAttribute('open', '')
  await firstProcessDisclosure.locator(':scope > summary').click()
  ...
  const guidedDisclosure = dialog.locator('details.assistant-work-plan-details').last()
  await expect(guidedDisclosure).not.toHaveAttribute('open', '')
  await guidedDisclosure.locator(':scope > summary').click()
  await expect(guidedPlan).toBeVisible()
