...
test('TEST-AI-P03-RENDER-01 presents a readable result and suppresses malformed charts', async ({ page }) => {
  ...
  const metrics = assistant.locator('.erumi-metrics-grid')
  await expect(metrics).toBeVisible()
  await expect(metrics.locator('.erumi-metric')).toHaveCount(2)
  await expect(metrics.locator('.erumi-metric').filter({ hasText: 'Tổng task' })).toContainText('28')
  await expect(metrics.locator('.erumi-metric').filter({ hasText: 'Quá hạn' })).toContainText('20')
  await expect(assistant.locator('.erumi-chart-card')).toHaveCount(0)
